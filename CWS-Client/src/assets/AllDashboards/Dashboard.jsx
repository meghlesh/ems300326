  ...
  axios
    ...
    .then((res) => {
      ...
      if (!userRole || !userName || !userId) {
        console.warn("⚠️ Invalid user data from /me");
        localStorage.clear();
      ...
      const currentPath = `/dashboard/${userRole}/${userName}/${userId}`;

      if (!localStorage.getItem("lastEMSRoute")) {
        localStorage.setItem("lastEMSRoute", currentPath);
        setLastEMSRoute(currentPath);
      }
    })
    .catch((err) => {
...
const fetchNotifications = async () => {
  ...
  path="resignation"
  element={
    ["hr","admin", "ceo", "coo", "md"].includes(role) ? (
      <Resignation />
    ) : (
